...
export interface ResponseListaRolesCatalogo {
  ok: boolean;
  message: string;
  data: DatumRoles[];
}

export interface DatumRoles {
  id: string;
  nombre: string;
  descripcion: string;
}
...
export const listaRolesCatalogo = async (): Promise<
  DatumRoles[] | undefined
> => {
  try {
    const { data }: AxiosResponse = await api.get(
      "/control-usuarios/roles/show-lista"
    );
